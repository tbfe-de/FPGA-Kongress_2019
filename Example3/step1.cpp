...
>
class QuickSine {
	std::array<DataType, LookupEntries> lookup;
...
public:
	...
	DataType operator()(int deg) const {
	...
	const DataType* rawTableData() const { return lookup.data(); }
	std::size_t rawTableSize() const { return lookup.size(); }
};
